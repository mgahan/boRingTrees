...
#' 
#'Compute rolling calcs such as 'count','sum','min','max', or 'median'
#'@param data A data file with fields: dates and a varible to pivot off of (by variable)
...
#'@param stat Default is "count". Can also use "mean", "median", "min", "max"
...
RollingByCalcs <- function(data,bylist,dates,target=NULL,
                           lower,upper,incbounds=T,stat="count",na.rm=T,cores=1){
  tic <- Sys.time()
  ...
  #Error checking
  if (stat!="count" & is.null(target)){stop("If stat!='count', then target cannot be null")}
  ...
  data$Roll.Val <- mcmapply(FUN = function(RD, NUM) {
    ...
    #sum((between(x=d,lower=lower,upper=upper,incbounds=incbounds))*Ref$Compare_Value[[NUM]])
    true.vals <- between(x=d,lower=lower,upper=upper,incbounds=incbounds)
    
    if (stat=="count"){
      out <- length(Ref$Compare_Value[[NUM]][true.vals])
    } else if (stat=="sum"){
      out <- sum(Ref$Compare_Value[[NUM]][true.vals],na.rm=na.rm)
    } else if (stat=="median"){
      out <- median(Ref$Compare_Value[[NUM]][true.vals],na.rm=na.rm)
    } else if (stat=="max"){
      out <- max(Ref$Compare_Value[[NUM]][true.vals],na.rm=na.rm)
    } else if (stat=="min"){
      out <- min(Ref$Compare_Value[[NUM]][true.vals],na.rm=na.rm)
    } else {
      stop("stat can only take on values 'count','sum','median','max','min'")
    }
    return(out)
